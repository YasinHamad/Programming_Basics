...
#define SIZE 50
/* --------------------------------- FUNCTIONS --------------------------------- */
int CountLetterInStringIgnoreCase(string source, char c) {
	char c_inverted = c;
	manipulators::string_utilities::InvertChar(c_inverted);

	int counter = 0;
	for (int i = 0; i < source.length(); i++) {
		if (source[i] == c || source[i] == c_inverted) counter++;
	}
	return counter;
}
...
int main()
{
	while (true) {
		...
		/* --------------------------------- PRINT --------------------------------- */
		cout << CountLetterInString(test_str, c) << endl;
		cout << CountLetterInString(test_str, c, false) << endl;
		//cout << c;
