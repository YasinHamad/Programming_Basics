...
public:
	...
	template <typename element_type>
	static int SearchInArray(element_type arr[], int size, element_type looking_for) {
	...
	//}
	 ...
	 //-1 means does not exist
	template <typename element_type>
	static int SearchInVector(vector<element_type> vec, element_type looking_for) {
		int counter = 0;
		for (const element_type& i : vec) {
			if (i == looking_for) {
				return counter;
			}
			counter++;
