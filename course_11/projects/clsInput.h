...
public:
	...
	static element_type ReadNumber(string message, string error_message = "Invalid Number, enter a valid one: ") {
		element_type number;
		cout << message;
		cin >> number;
		while (cin.fail()) {
	...
	template <typename element_type>
	static element_type ReadNumberInRange(element_type from, element_type to, string message, string error_message = "Number not in range, enter another one: ") {
		element_type number = ReadNumber<element_type>(message, error_message);

		while (!clsValidate::IsNumberBetween<element_type>(number, from, to)) {
			cout << error_message;
			number = ReadNumber<element_type>("", error_message);
		}
		return number;
	}
	...
	static void ReadArrayElements(int arr[], int& size) {
		size = ReadNumberInRange<int>(1, INT_MAX, "Enter array size: ", "The size should be positive, enter another one: ");
		for (int i = 0; i < size; i++) {
			arr[i] = ReadNumber<int>("Element [" + to_string(i + 1) + "] : ");
		}
	}
	static void ReadVectorElements(vector <int>& vector) {
		...
		while (check == 'y' || check == 'Y') {
			new_element = ReadNumber<int>("Enter an element: ");
