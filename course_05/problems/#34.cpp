...
void SearchInArray(int arr[], int size, int looking_for) {
	cout << "The number you are looking for is : " << looking_for << endl;
	bool flag = false;

	for (int i = 0; i < size; i++) {
		if (arr[i] == looking_for) {
			cout << "The number are found at position : " << i << endl;
			flag = true;
		}
	}
	if (!flag) cout << "The number is not found :-(" << endl;
}
int SearchInArray2(int arr[], int size, int looking_for) {
	for (int i = 0; i < size; i++) {
		if (arr[i] == looking_for) {
			return i;
		}
...
int main()
{
	...
	while (1)
	{
		...
		int looking_for = ReadNum("Enter a number to search: ");

		SearchInArray(arr, size, looking_for);
