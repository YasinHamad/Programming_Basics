...
void PrintResultsForPerfectNumber(int N) {
	...
	if (IsPerfect(N)) cout << N << " is perfect number." << endl;
	//else cout << N << " is not perfect number.";
}

void PrintResultsForPerfectNumbers(int N) {

	for (int i = 1; i <= N; i++) PrintResultsForPerfectNumber(i);
}
...
int main()
{
	...
	while (1) 
	{
		...
		PrintResultsForPerfectNumbers(ReadPositiveNum("Enter a positive number: "));
